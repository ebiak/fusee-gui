...
class LinuxBackend(HaxBackend):
    ...
    def print_warnings(self):
        ...
        log.append("A good way to ensure you're likely using an XHCI backend is to plug your")
        log.append("device into a blue 'USB 3' port.\n")
...
class WindowsBackend(HaxBackend):
    ...
    def find_device(self, Vid, Pid):
        ...
        if ret == 0:
            log.append('LibK_LoadDriverAPI(): Error')
            return None
...
class RCMHax:
    ...
    def __init__(self, wait_for_device=False, os_override=None, vid=None, pid=None, override_checks=False):
        ...
        try:
            ...
            log.append("It doesn't look like we support your OS, currently. Sorry about that!\n")
            sys.exit(-1)
        ...
        if self.dev is None:
            ...
            if wait_for_device:
                log.append("Waiting for a TegraRCM device to come online...")
        ...
        log.append("Identified a {} system; setting up the appropriate backend.".format(self.backend.BACKEND_NAME))
...
def do_hax(arguments):
    ...
    try:
        ...
        log.append("Found a Tegra with Device ID: {}".format(device_id))
    except OSError as e:
    ...
    if len(payload) > length:
        ...
        log.append("ERROR: Payload is too large to be submitted via RCM. ({} bytes larger than max).".format(size_over))
    ...
    log.append("Smashing the stack...")
    try:
        switch.trigger_controlled_memcpy()
    except ValueError as e:
        log.append(str(e))
    except IOError:
        log.append("The USB device stopped responding-- sure smells like we've smashed its stack. :)")
        log.append("Launch complete!")
    return(log)
    ...
    try:
        if sys.argv[1]:
            ...
            if file.lower().endswith(".bin"):
                ...
                log.append("File Type Not Supported")
    except:
        log.append("No Arguments Recived!")